...
navset_card_tab(
  nav_panel("Arbitrage Opportunities",
    card(
      card_header("Current Arbitrage Opportunities"),
      card_body(
        if(nrow(all_arbs) > 0) {
          DT::datatable(
            all_arbs,
          ...
          p("No arbitrage opportunities found at this time.")
        }
      )
    )
  ),
  
  nav_panel("Odds Outliers",
    card(
      card_header("Odds Outliers - Biggest Probability Differences"),
      card_body(
        if(nrow(all_outliers) > 0) {
          ...
          p("No significant odds outliers found.")
        }
      )
    )
  )
